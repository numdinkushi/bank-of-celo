...
export const RewardItem: React.FC<RewardItemProps & { onRedeem: (id: string) => void; }> = ({
  id,
  title,
  amount,
  description,
  ...
  return (
    <div className={`relative bg-gradient-to-br ${colors.bg} backdrop-blur-sm rounded-2xl p-6 mb-4 border ${colors.border} ${status === 'available' ? `shadow-xl ${colors.glow}` : ''} transition-all duration-300 ${status === 'available' ? 'hover:scale-[1.02] hover:shadow-2xl' : ''}`}>

      {/* Rarity indicator */}
      {rarity !== 'common' && (
        <div className="absolute top-3 right-3">
          <div className={`${colors.text} flex items-center gap-1 text-xs font-bold`}>
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start space-x-4 flex-1">
          <div className={`${colors.bg} p-3 rounded-xl ${colors.border} border backdrop-blur-sm`}>
            {icon}
          </div>
          <div className="flex-1">
            ...
            <div className={`${colors.text} font-bold text-xl mb-2`}>{amount}</div>
